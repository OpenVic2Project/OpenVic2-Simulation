...
void MapDefinition::_trace_river(BMP& rivers_bmp, ivec2_t start, river_t& river) {
	struct TraceSegment {
		ivec2_t point;
		uint8_t direction; /* 0 - start, 1 - up, 2 - down, 3 - left, 4 - right */
	};
	...
	while (!stack.empty()) {
		...
		while(true) {
			...
			uint8_t new_segment_direction;
			bool new_segment_found = false;

			// cleaner, clearer, and less repetitive
			struct Neighbour {
				ivec2_t offset;
			...
			for (const auto& neighbour : neighbours) {
                ...
                if (neighbour_color == size + 1) {
                    points.push_back(neighbour_pos);
