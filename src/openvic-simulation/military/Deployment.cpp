...
bool DeploymentManager::load_oob_file(
	...
	const auto leader_callback = [&leaders, &definition_manager, &general_count, &admiral_count](ast::NodeCPtr node) -> bool {
		...
		// Default cases for leader personality and background match vic2 behaviour of ignoring invalid traits. Vic2 has a weird noTrait state which is undesirable, so instead we give them the first applicable trait we can find.
		if (leader_personality != nullptr && !leader_personality->is_personality_trait()) {
			LeaderTrait const* def_personality = nullptr;
			for (const auto& trait : definition_manager.get_military_manager().get_leader_trait_manager().get_leader_traits()) {
				if (def_personality == nullptr && trait.is_personality_trait()) { def_personality = &trait; }
			}
			Logger::warning(
				"Leader ", leader_name, " has personality \"", leader_personality->get_identifier(),
				"\" which is not a personality trait! Assigned default personality trait \"", def_personality->get_identifier(), "\"."
			);
			leader_personality = def_personality;
		}

		if (leader_background != nullptr && !leader_background->is_background_trait()) {
			LeaderTrait const* def_background = nullptr;
			for (const auto& trait : definition_manager.get_military_manager().get_leader_trait_manager().get_leader_traits()) {
				if (def_background == nullptr && trait.is_background_trait()) { def_background = &trait; }
			}
			Logger::warning(
				"Leader ", leader_name, " has background \"", leader_background->get_identifier(),
				"\" which is not a background trait! Assigned default background trait \"", def_background->get_identifier(), "\"."
			);
			leader_background = def_background;
		}
