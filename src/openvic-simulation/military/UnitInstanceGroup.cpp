	...
	if (leader != new_leader) {
		...
		if (leader != nullptr) {
			if (leader->get_branch() == branch) {
				...
				Logger::error(
					"Trying to assign ", UnitType::get_branch_name(leader->get_branch()), " leader \"",
					leader->get_name(), "\" to ", UnitType::get_branch_name(branch), " unit group \"", name, "\""
				);
				ret = false;
				leader = nullptr;
			}
