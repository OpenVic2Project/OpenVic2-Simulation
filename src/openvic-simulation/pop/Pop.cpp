...
void Pop::pop_tick() {
	...
	PopType const& type_never_null = *type;
	ProvinceInstance const& location_never_null = *location;
			...
			*good_definition,
			...
			[this, good_definition](const BuyResult buy_result) -> void {
				...
				#define CONSUME_NEED(need_category)\
					...
					if (need_category##_needs.contains(good_definition)) {\
						const fixed_point_t desired_quantity = need_category##_needs[good_definition];\
